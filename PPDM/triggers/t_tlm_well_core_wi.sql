...
------------------------------------------------------------------------------------------------------------------------
----drop trigger t_tlm_well_core_wi;

create or replace trigger t_tlm_well_core_wi 
  after update or insert or delete
  on ppdm.TLM_WELL_CORE for each row
...
begin
  ...
  /*
  -------------------------------------------------------------------------------------
  Trigger:    t_tlm_well_core_wi
  Purpose:    This trigger is for table TLM_WELL_CORE
              Well inventory type: Well Core 
  ...
  */
  ...
  if inserting or updating then
    aq_well_inventory.aq_enqueue_wi_uwi(:new.uwi, 'CORES');
    --ppdm_admin.tlm_process_logger.info('--WELL CORE trigger for advanced queuing - insert or update ' || :new.uwi);
  end if;
  
  if updating or deleting then
    aq_well_inventory.aq_enqueue_wi_uwi(:old.uwi, 'CORES');
    --ppdm_admin.tlm_process_logger.info('--WELL CORE trigger for advanced queuing - update or delete ' || :old.uwi);
  end if;
...
alter trigger T_TLM_WELL_CORE_AIUD2 disable;
alter trigger T_TLM_WELL_CORE_AIUD1 disable;
/
