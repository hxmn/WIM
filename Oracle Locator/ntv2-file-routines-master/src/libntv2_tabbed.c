   ...
   NTV2_REC *rec)
{
   ...
   for (i = 0; i < rec->num; i++)
   {
      ...
      fprintf(fp, "%-16s%s", D(gs.f_lat_shift), charSpacer );
      fprintf(fp, "%-16s%s", D(gs.f_lon_shift), charSpacer );
      fprintf(fp, "%-16s%s", D(gs.f_lat_accuracy), charSpacer );
      fprintf(fp, "%s\n",  D(gs.f_lon_accuracy), charSpacer );
   }
