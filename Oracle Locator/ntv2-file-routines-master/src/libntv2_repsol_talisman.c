   ...
   NTV2_REC *rec)
{
   ...
   /* 
        10/22/2015 kxedward Repsol (Talisman)
        Added spacer so data is not munged together
   */
   const char* charSeperator = "\t";
   ...
   for (i = 0; i < rec->num; i++)
   {
      ...
      fprintf(fp, "%-16s%s", D(gs.f_lat_shift), charSeperator );
      fprintf(fp, "%-16s%s", D(gs.f_lon_shift), charSeperator );
      fprintf(fp, "%-16s%s", D(gs.f_lat_accuracy), charSeperator );
      fprintf(fp, "%s\n",  D(gs.f_lon_accuracy), charSeperator );
   }
