...
--   
    ...
    FUNCTION last_chg_by (input_uwi in VARCHAR2, input_max_date in DATE) RETURN VARCHAR2
    IS
      --  Determines who last updated the well, regardless of the precedence order
      v_by VARCHAR2(30);

    BEGIN 

      -- Use max in case more than 1 row has the same timestamp
      select max(row_changed_by) INTO v_by
        from well_version 
