...
Public Class DBAccess
  ...
  Public Function GetUserPermissions(ByVal iuserID As String) As DataSet
    ...
    sSql.Append("SELECT S.SOURCE, S.SHORT_NAME || ' (' || S.SOURCE || ')' as SHORT_NAME")
    ...
    'Setup table name and query for Reactivate permission
    sTables.Append(USER_REACTIVATE_PERMISSION)
    sTables.Append(";")
  ...
  Public Function FindWellsStdUOM(ByVal iWellList As String, ByVal iTableName As String) As DataTable
    ...
    sSql.Append("SELECT DISTINCT W.UWI, W.IPL_UWI_LOCAL, RS.SOURCE, RS.SHORT_NAME || ' (' || RS.SOURCE || ')' SOURCE_SHORT_NAME,")
    sSql.Append(" W.ACTIVE_IND, RC.LONG_NAME COUNTRY,")
    ...
    sSql.Append(" LEFT OUTER JOIN PPDM.WELL_NODE SWN ON SWN.NODE_ID = W.SURFACE_NODE_ID AND SWN.LATITUDE = W.SURFACE_LATITUDE AND SWN.LONGITUDE = W.SURFACE_LONGITUDE")
    sSql.Append(" LEFT OUTER JOIN PPDM.CS_COORDINATE_SYSTEM SCS ON SCS.COORD_SYSTEM_ID = SWN.GEOG_COORD_SYSTEM_ID")
    sSql.Append(" LEFT OUTER JOIN PPDM.WELL_NODE BWN ON BWN.NODE_ID = W.BASE_NODE_ID AND BWN.LATITUDE = W.BOTTOM_HOLE_LATITUDE AND BWN.LONGITUDE = W.BOTTOM_HOLE_LONGITUDE")
    sSql.Append(" LEFT OUTER JOIN PPDM.CS_COORDINATE_SYSTEM BCS ON BCS.COORD_SYSTEM_ID = BWN.GEOG_COORD_SYSTEM_ID")
    sSql.Append(" WHERE W.UWI IN (")
    sSql.Append(sbWellList)
    sSql.Append(")")
    sSql.Append(" AND UPPER(W.ACTIVE_IND) = 'Y'")
    sSql.Append(" ORDER BY W.UWI")
  ...
  Public Function GetVersionOrigUOM(ByVal iTLMID As String, ByVal iSource As String) As DataSet
    ...
    Else
      ' update the SOURCE_NAME to append the SOURCE in parens
      dsRealData.Tables("WELL_VERSION").Rows(0)("SOURCE_NAME") _
        = dsRealData.Tables("WELL_VERSION").Rows(0)("SOURCE_NAME") _
        & " (" _
        & dsRealData.Tables("WELL_VERSION").Rows(0)("SOURCE") _
        & ")"
    End If
  ...
  Public Function FindVersionsStdUOM(ByVal iWellList As String, ByVal iTableName As String) As DataTable
    ...
    sSql.Append("SELECT DISTINCT W.UWI, W.IPL_UWI_LOCAL, W.SOURCE, RS.SHORT_NAME || ' (' || RS.SOURCE || ')' SOURCE_SHORT_NAME,")
    sSql.Append(" W.ACTIVE_IND, RC.LONG_NAME COUNTRY,")
    sSql.Append(" W.WELL_NAME WELL_NAME, W.PLOT_NAME, RWS.SHORT_NAME CURRENT_STATUS, TO_CHAR(W.SPUD_DATE, 'dd/mm/yyyy') SPUD_DATE,")
    sSql.Append(" TO_CHAR(W.RIG_RELEASE_DATE, 'dd/mm/yyyy') RIG_RELEASE_DATE, W.GROUND_ELEV GROUND_ELEV,")
    sSql.Append(" W.KB_ELEV KB_ELEV, W.DRILL_TD DRILL_TD,")
    ...
    sSql.Append(" LEFT OUTER JOIN PPDM.R_COUNTRY RC ON W.COUNTRY = RC.COUNTRY")
    sSql.Append(" LEFT OUTER JOIN PPDM.R_WELL_STATUS RWS ON W.CURRENT_STATUS = RWS.STATUS")
    sSql.Append(" LEFT OUTER JOIN PPDM.WELL_NODE_VERSION SWNV ON SWNV.NODE_ID = W.SURFACE_NODE_ID AND SWNV.SOURCE = W.SOURCE AND SWNV.LATITUDE = W.SURFACE_LATITUDE AND SWNV.LONGITUDE = W.SURFACE_LONGITUDE")
    sSql.Append(" LEFT OUTER JOIN PPDM.CS_COORDINATE_SYSTEM SCS ON SCS.COORD_SYSTEM_ID = SWNV.GEOG_COORD_SYSTEM_ID")
    sSql.Append(" LEFT OUTER JOIN PPDM.WELL_NODE_VERSION BWNV ON BWNV.NODE_ID = W.BASE_NODE_ID AND BWNV.SOURCE = W.SOURCE AND BWNV.LATITUDE = W.BOTTOM_HOLE_LATITUDE AND BWNV.LONGITUDE = W.BOTTOM_HOLE_LONGITUDE")
    sSql.Append(" LEFT OUTER JOIN PPDM.CS_COORDINATE_SYSTEM BCS ON BCS.COORD_SYSTEM_ID = BWNV.GEOG_COORD_SYSTEM_ID")
    sSql.Append(" WHERE W.UWI IN (")
    sSql.Append(sbWellList)
    sSql.Append(")")
    'sSql.Append(" AND UPPER(W.ACTIVE_IND) = 'Y'")
